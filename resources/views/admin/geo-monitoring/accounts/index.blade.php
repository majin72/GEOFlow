                        ...
                        @foreach ($accounts as $account)
                            ...
                            <tr class="{{ ($stats['schedulable'] ?? false) ? '' : 'bg-amber-50/40' }}">
                                ...
                                <td class="px-4 py-4 text-right text-sm space-x-3">
                                    ...
                                    <form method="post" action="{{ route('admin.geo-monitoring.accounts.delete', ['accountId' => $account->id]) }}" class="inline" onsubmit="return confirm(@js(__('admin.geo_monitoring.confirm_delete_account', ['name' => $account->label])));">
                                        @csrf
