...
| 機能 | 説明 |
|------|------|
| 🤖 マルチモデル生成 | OpenAI 互換 API で複数プロバイダに接続可能 |
| 📦 バッチタスク | 作成・スケジュール・キュー・再試行。**Laravel Horizon** による監視も可 |
| 🗂 素材の一元管理 | タイトル、キーワード、画像、ナレッジ、プロンプト |
| 📋 レビューと公開 | 下書き → レビュー → 公開、自動公開の設定も可 |
| 🔍 検索向け表示 | SEO メタ、OG、構造化データ |
| 🎨 フロントとテーマ | 記事サイトと管理画面のサイト／テンプレート設定 |
| ⚡ リアルタイム | **Laravel Reverb** 等（`.env` で有効化） |
...
主な流れ：管理でモデル・プロンプト・素材を設定 → タスク作成とキュー投入 → Worker が生成 → 下書き〜公開 → フロントで SEO 付き記事を表示。
